  ...
  return (
    <div className="no-print">
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 w-16 h-16 rounded-full bg-[#1a2d5a] text-white flex items-center justify-center cursor-pointer shadow-2xl border-2 border-[#c0392b] z-50 focus:outline-none hover:scale-105 transition-transform"
        whileTap={{ scale: 0.95 }}
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              <X className="w-6 h-6 text-[#c0392b]" />
            </motion.div>
          ) : (
            <motion.div
              key="monogram"
        ...
        <span className="absolute inset-0 rounded-full border border-[#c0392b] opacity-60 scale-110 animate-ping pointer-events-none" style={{ animationDuration: "3s" }} />
      </motion.button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-[#0f1e3d] backdrop-blur-sm z-40"
            />
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-full max-w-md bg-[#1a2d5a] text-white border-l-2 border-[#c0392b] shadow-2xl z-40 flex flex-col p-8 overflow-y-auto"
            >
              {/* Drawer Header */}
              <div className="flex flex-col items-center gap-4 py-8 border-b border-[#c0392b]/30 text-center shrink-0">
                ...
                <div className="flex flex-col font-serif">
                  <h2 className="text-xl font-bold text-[#e57373] tracking-wide">
                    {t("Shree Alankar Public", "श्री अलंकार")}
                  </h2>
                    ...
                    >
                      <Link
                        ...
                        className={`flex flex-col py-1.5 transition-all group pl-4 relative ${
                          isActive
                            ? "text-[#e57373] font-bold border-l-4 border-[#c0392b] -ml-4 pl-4"
                            : "text-white/80 hover:text-[#e57373] hover:translate-x-2"
                        }`}
                      >
                        <span className="text-xl md:text-2xl leading-none">
                          {t(item.labelEn, item.labelNp)}
                        </span>
                        <span className="text-xs uppercase tracking-widest text-[#c0392b]/60 group-hover:text-[#c0392b]/90 transition-colors font-sans mt-0.5">
              ...
              <div className="mt-auto pt-6 border-t border-[#c0392b]/30 flex items-center justify-between shrink-0">
                {/* Language switch */}
                <button
                  onClick={handleLangToggle}
                  className="flex items-center gap-2 py-2 px-4 rounded border border-[#c0392b]/40 text-[#c0392b] hover:bg-[#c0392b] hover:text-white transition-all cursor-pointer font-sans text-xs font-semibold uppercase tracking-wider"
                >
