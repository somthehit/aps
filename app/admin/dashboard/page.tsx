...
import CalendarImageManager from "@/components/admin/CalendarImageManager";
import CalendarDataManager from "@/components/admin/CalendarDataManager";

type Tab = "overview" | "notices" | "milestones" | "admissions" | "results" | "settings" | "hero" | "faculty" | "gallery" | "events" | "messages" | "academicPrograms" | "calendarImages" | "calendarData";
...
It is my extreme privilege and honor to serve as the Principal of Shree Alankar Public School, Motibasti, Kanchanpur. Ever since our inception in 2073 BS, our school has undergone massive shifts from a modest rural basic school to a full-fledged private secondary educational institution supporting diverse curriculums in Science, Humanities, and Education.
...
कञ्चनपुर जिल्लाको पुनर्वास नगरपालिकास्थित श्री अलंकार माध्यमिक विद्यालयको प्रधानाध्यापकका रूपमा यहाँहरूसँग जोडिन पाउँदा म अत्यन्तै गौरवान्वित छु। वि.सं. २०६६ सालमा सामान्य प्राथमिक पाठशालाको रूपमा स्थापना भएको यो विद्यालय आज क्षेत्रकै उत्कृष्ट सामुदायिक माध्यमिक विद्यालय बन्न सफल भएको छ।
...
धन्यवाद। यहाँहरू सबैको शैक्षिक यात्रा सुखद र सफल रहोस्।`,
    ...
    addressEn: "Punarbas-8, Prithvibasti, Kanchanpur, Nepal",
    addressNp: "पुनर्वास-८, पृथ्वीबस्ती, कञ्चनपुर, नेपाल",
    calendar_year: "२०८३",
    totalStudents: "847+",
    totalStaff: "42",
    establishedYearBS: "2066",
    schoolClasses: "1–10",
  });
  ...
  const handleExportCSV = () => {
    ...
    const rows = inquiries
      .map((inq) => `SAPS-2083-${String(inq.id).padStart(2,'0')},"${inq.student_name}","${inq.guardian_name}",${inq.grade_applying},${inq.phone},${inq.email || ''},"${inq.address || ''}",${inq.status},${new Date(inq.submitted_at).toLocaleDateString()}`)
      .join("\n");
    const blob = new Blob([headers + rows], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `SAPS_Admissions_2083.csv`;
    link.click();
  };
    ...
    <div className="flex-1 flex flex-col md:flex-row bg-[#102419]/5 min-h-screen text-slate-800">
      ...
      <aside className="w-full md:w-72 bg-[#1a3a2a] text-white flex flex-col border-r border-[#c9a227] shrink-0 no-print">
        ...
        <nav className="flex-grow py-6 px-4 flex flex-col gap-2 font-sans text-xs uppercase tracking-wider font-semibold text-left">
          ...
          <button
            onClick={() => setActiveTab("calendarImages")}
            className={`w-full p-3.5 rounded flex items-center gap-3 cursor-pointer transition-all ${
              activeTab === "calendarImages"
                ? "bg-[#c9a227]/20 text-[#c9a227] border-l-4 border-[#c9a227]"
                : "text-white/70 hover:bg-white/5 hover:text-white"
            }`}
          >
          ...
          <button
            onClick={() => setActiveTab("calendarData")}
            className={`w-full p-3.5 rounded flex items-center gap-3 cursor-pointer transition-all ${
      ...
      <main className="flex-grow p-6 md:p-8 overflow-y-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#1a3a2a]/10 pb-4 mb-8 text-left">
          <div>
            ...
            <p className="text-xs text-slate-500 font-sans mt-0.5">
              Welcome back to SAPS Management Console. Here is your active database.
            </p>
          </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left font-sans text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {inquiries.map((inq) => {
                      const appId = `SAPS-2083-${String(inq.id).padStart(2, '0')}`;
                ...
                <ImageUploader
                  onUpload={(url) => {
                    fetch('/api/hero-slides', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ image_url: url, caption: newHeroCaption || null, display_order: heroSlides.length }),
                    }).then(r => r.ok ? r.json() : null).then(j => {
                      if (j) { setHeroSlides([...heroSlides, j.data]); setNewHeroCaption(''); }
                    });
                  }}
                  folder="hero-slides"
            ...
            <form onSubmit={handleSettingsSave} className="flex flex-col gap-6 font-sans text-xs sm:text-sm">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4 accent-[#1a3a2a] cursor-pointer"
                />
                <label htmlFor="maintenanceSwitch" className="cursor-pointer text-[#8b1a1a] font-semibold text-xs">Enable System Maintenance Mode</label>
              </div>
        ...
        {activeTab === "calendarImages" && <CalendarImageManager />}
        {activeTab === "calendarData" && <CalendarDataManager />}
